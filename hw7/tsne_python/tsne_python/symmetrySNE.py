...
def symSNE(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0):
    ...
    for iter in range(max_iter):
        ...
        num = np.exp(-(np.add(np.add(num, sum_Y).T, sum_Y)))  # exp(-||a-b||^2) ssne
        ...
        PQ = P - Q
        for i in range(n):
            # dY[i, :] = np.sum(np.tile(PQ[:, i] * num[:, i], (no_dims, 1)).T * (Y[i, :] - Y), 0)  # tsne
    ...
    pylab.clf()
    pylab.scatter(Y[:, 0], Y[:, 1], 20, labels)
    pylab.savefig(f'symSNE_perplexity{perplexity}_final.png')
    pylab.show()

    fig = visualize_similarity_distributions(P, Q)
    plt.savefig(f'similarity_dist_symSNE.png',
...
def create_gif():
    ...
    images[0].save('symSNE_animation.gif',
                   save_all=True,
